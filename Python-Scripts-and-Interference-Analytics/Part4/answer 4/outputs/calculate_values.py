...
def read_jobs(number_run):
    jobs = dict()
    with open("jobs_1s" + number_run + ".txt", 'r') as file:
        for line in file:
            elements = line.split()
...
def read_mcperf(number_run):
    ...
    timestamp_end = 0
    table = []
    with open("mcperf_1s" + number_run + ".txt", 'r') as file:
        for line in file:
            elements = line.split()
            if len(elements) > 2:
...
def main():
    ...
    for index in range(1,4):
        ...
        total_violate += violate / len(p95_list)
        print("mcperf values counted: " + str(len(p95_list)))
    print("avg violate: " + str(total_violate/3))
