...
class DockerController:
    ...
    def __init__(self, logger):
        ...
        blackscholes = self.create_parsec_job("2", "2,3", "blackscholes", "parsec", "anakli/cca:parsec_blackscholes")
        # dict {String: [number_of_threads, number_of_cores]}
        self.jobs[blackscholes] = ["2", "2,3"]
        canneal = self.create_parsec_job("4", "2,3", "canneal", "parsec", "anakli/cca:parsec_canneal")
        self.jobs[canneal] = ["4", "2,3"]
        dedup = self.create_parsec_job("1", "2,3", "dedup", "parsec", "anakli/cca:parsec_dedup")
        self.jobs[dedup] = ["1", "2,3"]
        ferret = self.create_parsec_job("3", "2,3", "ferret", "parsec", "anakli/cca:parsec_ferret")
        self.jobs[ferret] = ["3", "2,3"]
        freqmine = self.create_parsec_job("4", "2,3", "freqmine", "parsec", "anakli/cca:parsec_freqmine")
        self.jobs[freqmine] = ["4", "2,3"]
        radix = self.create_parsec_job("1", "2,3", "radix", "splash2x", "anakli/cca:splash2x_radix")
        self.jobs[radix] = ["1", "2,3"]
        vips = self.create_parsec_job("2", "2,3", "vips", "parsec", "anakli/cca:parsec_vips")
        self.jobs[vips] = ["2", "2,3"]
        self.queue = [blackscholes, canneal, dedup, ferret, freqmine, radix, vips]
    ...
    def update_cores(self, container, cores):
        container.reload()
        if container.status == "exited":
            return
        container.update(cpuset_cpus=cores)
        print("CHANGED CORES" + container.name + cores)
        self.logger.update_cores(container.name, cores)
        '''
        if cores == "2,3":
            container.pause()
            self.logger.job_pause(container.name)
        '''
        self.jobs[container][1] = cores
    ...
    def update(self, number_of_cores_memcached):
        ...
        for job in self.queue:
            ...
            if cores_changed and number_of_cores_memcached == 2:
                self.update_cores(job, "2,3")
            elif cores_changed and number_of_cores_memcached == 1:
                self.update_cores(job, "1,2,3")
