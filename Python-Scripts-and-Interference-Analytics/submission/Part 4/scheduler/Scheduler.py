    ...
    def add_job(self):
        ...
        for job in self.running:
            if self.jobs[job][2] == "short":
                run_short_job = False
            elif self.jobs[job][2] == "long":
                run_long_job = False
        if len(self.running) < 2 and run_long_job and len(self.queue_long_jobs) != 0:
            new = self.queue_long_jobs[0]
            self.run_container(new)
            self.running.append(new)
            self.queue_long_jobs.remove(new)
            self.update_cores(new)
        if len(self.running) < 2 and run_short_job and len(self.queue_short_jobs) != 0:
            new = self.queue_short_jobs[0]
            self.run_container(new)
            self.running.append(new)
            self.queue_short_jobs.remove(new)
            self.update_cores(new)
        if len(self.running) < 2 and len(self.queue_long_jobs) != 0:
            new = self.queue_long_jobs[0]
            self.run_container(new)
            self.running.append(new)
            self.queue_long_jobs.remove(new)
            self.update_cores(new)
        if len(self.running) < 2 and len(self.queue_short_jobs) != 0:
            new = self.queue_short_jobs[0]
            self.run_container(new)
            self.running.append(new)
            self.queue_short_jobs.remove(new)
